...
class RSAKey {
    ...
    public String toString() {
    	return "d: " + d + "e: " + e + "n: " + n + "bitLength: "+ bitLength;
    }
}
...
public class RSA {
    ...
    public static RSAKey generateKeys (int bitLength, int seed) throws Exception {
      ...
      //Choose two large random prime numbers p and q- 2048 bit
        //Look at code in slides for that conversion
        
        BigInteger p = new BigInteger(bitLength, 25, random);
        BigInteger q = new BigInteger(bitLength, 25, random);
        
        //compute n = p * q, should be about twice as many bits
        BigInteger n = (p).multiply(q);
        //choose random prime number e
        
        BigInteger e = new BigInteger(bitLength*2-10, 25, random);
    ...
    public static CipherText encrypt (RSAKey key, String plainText) throws Exception {
    	...
    	//encode message m into a number

 	   
 	   
 	   //figure out block size
 	   
 	   int maxBlockSize = (int)Math.floor((key.n.bitLength()-1)/8); // max block size
 	   int b = 8*(int)Math.floor(maxBlockSize/8); //num bits per block

 	   
 	   //convert to byte array
 	   byte[] chars = plainText.getBytes();
 	   

 	   int numBlocks = (int)Math.ceil((double)chars.length / b);
 	   
 	   //put into numBlocks blocks of length b
 	   BigInteger[] blocks = new BigInteger[numBlocks];
 	   ...
 	   for (int i = 0; i < numBlocks; i++) {
 			...
 			byte[] block = new byte[end-i*b];
 			System.arraycopy(chars, i*b, block, 0, end-i*b);
 	   ...
 	   for(int i=0;i<blocks.length;i++) {
 		   cipher.encryptedBlocks.add(blocks[i].modPow(key.e, key.n)); 
 	   }

 	   cipher.blockLength = b;
 	   cipher.fileLength = plainText.length();
    ...
    public static String decrypt (RSAKey key, CipherText cipherText) throws Exception {
    	...
    	//Use modPow m = c^d mod n or c.modPow(d,n) where c is encryptedBlocks[i]
 	   String word = "";
 	  
 	   //decrypt message using modPow
 	   for(int i=0;i<cipherText.encryptedBlocks.size();i++) {
 		   BigInteger c = cipherText.encryptedBlocks.get(i);
 		   BigInteger temp = c.modPow(key.d, key.n);	   	   
 		   String w = convertToASCII(temp);
 		   word+=w;
 	   }
	...
	public static RSAKey crack(RSAKey key) throws Exception {
		// your code here; a dummy return statement is put below to allow this to
		// compile
		...
		while (i.compareTo(key.n) <= 0) {
			if (key.n.mod(i).equals(new BigInteger("0"))) {
				BigInteger temp = key.n.divide(i);
				if (temp.isProbablePrime(25) && i.isProbablePrime(25) && (temp.bitLength() == i.bitLength())) {
					p = temp;
					q = i;
					break;
				}
    ...
    public static CipherText sign (RSAKey key, String plainText) throws Exception {
    	// determine the plainText's SHA2 Hash
 	   MessageDigest digest = MessageDigest.getInstance("SHA-256");
 	   byte[] hashedText = digest.digest(plainText.getBytes());
 	   String hex = convertHash(hashedText);
 	   //System.out.println(hex);
 	   // encrypt hash with private key (n,d) to get c
 	   key.e = key.d;
 	   CipherText cipher = encrypt(key, hex);
 	   // send concatenated message m = plainText + c
    ...
    public static boolean checkSign (RSAKey key, String plainText, CipherText signature) throws Exception {
    	//decript c using public key
    	key.d = key.e;
 	   String hash = decrypt(key, signature);
 	   //System.out.println(hash);
 	   //take hash of this to get h2
 	   MessageDigest digest = MessageDigest.getInstance("SHA-256");
 

 	   
 	   byte[] hashedText = digest.digest(plainText.getBytes());
 	   String textHex = (convertHash(hashedText));
 	   //compare to h1
 	   if(textHex.equals(hash)) {
 		   return true;
 	   }
